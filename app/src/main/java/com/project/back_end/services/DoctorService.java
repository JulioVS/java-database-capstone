...
public class DoctorService {
    ...
    @Transactional
    public List<String> getDoctorAvailability(@NonNull Long doctorId, @NonNull LocalDate date) {
    ...
    public int updateDoctor(Doctor doctor) {
        ...
        if (doctorId == null) {
            return -1; // Doctor ID is required for update
        }
    ...
    @Transactional
    public int deleteDoctor(@NonNull Long doctorId) {
    ...
    private List<Doctor> filterDoctorByTime(List<Doctor> doctors, String amOrPm) {
        ...
        List<Doctor> filteredDoctors = doctors.stream()
                .filter(doctor -> doctor.getAvailableTimes().stream()
                        .anyMatch(time -> isTimeInAmOrPm(time, amOrPm)))
                .collect(Collectors.toList());
    ...
    // My own auxiliary method to check if a time slot falls within AM or PM
    // time periods. This method is used in the filtering logic to determine if a
    // doctor's available time slots match the specified time period (AM/PM).
    private boolean isTimeInAmOrPm(String time, String amOrPm) {
        ...
        try {
            ...
            if (amOrPm.equalsIgnoreCase("AM")) {
                return (startHour >= 0 && startHour < 12) || (endHour > 0 && endHour <= 12);
            } else if (amOrPm.equalsIgnoreCase("PM")) {
                return (startHour >= 12 && startHour < 24) || (endHour > 12 && endHour <= 24);
            }
