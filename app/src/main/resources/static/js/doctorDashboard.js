...
document.getElementById("searchBar").addEventListener("input", (e) => {
  ...
  patientName = inputValue ? inputValue : "null"; // Set to "null" if empty
  loadAppointments(); // Reload appointments with the new filter
});

// "Today" button event listener
document.getElementById("todayButton").addEventListener("click", () => {
  selectedDate = new Date().toISOString().split("T")[0]; // Reset to today's date
  document.getElementById("datePicker").value = selectedDate; // Update date picker UI
  loadAppointments(); // Reload appointments for today
});

// Date picker event listener
document.getElementById("datePicker").addEventListener("change", (e) => {
  selectedDate = e.target.value; // Update selected date
...
async function loadAppointments() {
  try {
    ...
    errorRow.innerHTML = `<td colspan="4" class="text-center text-danger">Error loading appointments. Try again later.</td>`;
